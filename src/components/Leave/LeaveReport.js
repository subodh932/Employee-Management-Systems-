...
export default class LeaveReport extends React.Component {
  ...
  componentDidMount() {
    if (window.sessionStorage.getItem("user_level_id") == "2") {
      axios
        .get(
          `${config.api_url}/leaves/all-leaves/` +
      ...
      axios.get(`${config.api_url}/leaves/all-leaves/0`).then((res) => {
        const leaves = res.data;
        this.setState({ leaves });
        console.log(leaves);
      });
   ...
   */
  deleteData(id) {
    axios.delete(`${config.api_url}/leaves/${id}`).then((res) => {
      console.log(res);
                ...
                <tbody>
                  {this.state.leaves.map((leave, index) => (
                    <tr>
                      ...
                      <td className={leave.leave_status}>
                        {leave.leave_status}
                      </td>
                      <td>
                        ...
                        &nbsp;&nbsp;
                        <a
                          onClick={() => this.confirmatioBox(leave.leave_id)}
                          href="#!"
                        >
