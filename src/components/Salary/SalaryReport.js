...
export default class SalaryReport extends React.Component {
  ...
  download_slip = (salary_slip) => {
    window.location.replace(
      "http://127.0.0.1:8080/api/v1/salaries/salary_slip/" + salary_slip
    );
  };
  ...
  componentDidMount() {
    ...
    if (window.sessionStorage.getItem("user_level_id") == "2") {
      axios
        .get(
          `${config.api_url}/salaries/all-salaries/` +
   ...
   */
  deleteData(id) {
    axios.delete(`${config.api_url}/salaries/${id}`).then((res) => {
      console.log(res);
                  ...
                  {this.state.salarys.map((salary, index) => (
                    <tr>
                      ...
                      <td>
                        ...
                        &nbsp;&nbsp;
                        <a
                          onClick={() => this.download_slip(salary.salary_slip)}
                          href="#!"
                        >
